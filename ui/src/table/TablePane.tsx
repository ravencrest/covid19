...
const buildColumns = memoizeOne(
  (normalized: boolean, dataset: DataSets): Column<TableRow>[] => {
    const columns: Column<TableRow>[] = [
      {
        Header: (
          <Header tooltip='Increase in new cases since last data'>
            Change
          </Header>
        ),
        accessor: 'change',
...
export const TablePane = ({
  ...
  data: TableRow[];
  hideRecovered: DataSets;
  normalized: boolean;
}) => {
  const filteredColumns = buildColumns(normalized, hideRecovered);
  return (
